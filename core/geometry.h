...
template<size_t DIM,typename T>vec<DIM,T> operator-(vec<DIM,T> lhs) {
    vec<DIM,T> ret;
    for (size_t i=DIM; i--; ret[i] = -lhs[i]);
    return ret;
}

template<size_t DIM,typename T>vec<DIM,T> cwiseProduct(vec<DIM,T> lhs, vec<DIM,T> rhs) {
    vec<DIM,T> ret;
...
public:
    ...
    mat(std::initializer_list<vec<DimCols,T>> v) {
        auto beg = v.begin();
        for (int i = 0; i < v.size(); ++i, ++beg)
            set_row(i, *beg);
    }
