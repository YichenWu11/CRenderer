...
static void register_window_class()
{
	...
	wc.lpszClassName = "CRenderer";					//该窗口类的名字
...
int window_init(int width, int height, const char* title)
{
	...
	memset(window, 0, sizeof(window_t)); // 初始化
	window->is_close = 0;
	...
	//创建窗口
	window->h_window = CreateWindow(("CRenderer"), title,
	...
	memset(window->window_fb, 0, width * height);
...
void window_draw(TGAImage* framebuffer)
{
	int i, j;
	for (int i = 0; i < window->height; i++)
	{
		for (int j = 0; j < window->width; j++)
		{
			...
			TGAColor c = framebuffer->get(j, i);
